...
define(['underscore'], function(_) {
  ...
  return function Issue(issue) {
    ...
    var projeto = _.find(window.projects, function(x) {
      return x.id === issue.project_id;
    });
    ...
    self.EsforcoEstimado = function() {
      var temFilhos = _.filter(window.issues, function(x) {
        return x.parent_id === issue.id
      }).length > 0;
      return (!temFilhos && issue.estimated_hours) ? issue.estimated_hours : 0;
    }();

    self.EsforcoRealizado = function() {
      var entries = _.filter(window.timeEntries, function(x) {
        return x.issue_id === issue.id;
      });
      return _.reduce(entries, function(memo, num) {
        return memo + num.hours;
      }, 0);
    }();

    self.Tamanho = function() {
      var tamanho = 0;
      var tamanhoCField = _.find(window.customfields, function(x) {
        return x.name === 'Tamanho';
      });
      if (!tamanhoCField) return 0;
      var tamanhoCValue = _.find(window.customValues, function(x) {
        return x.customized_type === "Issue" && x.customized_id === issue.id && x.custom_field_id === tamanhoCField.id;
      });
    ...
    self.Tipo = _.find(window.trackers, function(x) {
      return x.id === issue.tracker_id;
    }).name;

    self.Status = _.find(window.statuses, function(x) {
      return x.id === issue.status_id
    }).name;
    ...
    self.CustomValues = function() {
      ...
      var cvalues = _.filter(window.customValues, function(x) {
        return x.customized_type === "Issue" && x.customized_id === issue.id;
      });
      ...
      _.each(cvalues, function(item, index) {
        ...
        if (cfield != null)
          arr.push({
            CustomField: cfield.name,
            CustomValue: item.value,
            Effort: self.EsforcoRealizado
          });
      });
